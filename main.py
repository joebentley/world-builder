...
if __name__ == "__main__":
    ...
    #               "floor is covered in a thin layer of hay, which gets progressively more blood-soaked as it "
    #               "progresses toward the northern iron door. A rusty bucket is on the floor.".format(_id1), batch=True)
    #
    ...
    #
    # command.parse("@add exit n {} {}".format(_id1, _id2), batch=True)
    # command.parse("@add exit s {} {}".format(_id2, _id1), batch=True)
    #
    # command.parse("@save world.json")

    command.parse("@load world.json")

    p.player.current_room = 1

    # p.player.current_room = _id1
    command.parse("look", player=p.player)
